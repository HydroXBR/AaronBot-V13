...
exports.int = async(client, interaction) => {
	...
	if(command == 'banlist'){
		...
		try{
			if(!db.get(`tk${interaction.guildId}`)) return messagerEphemeral(`Ops! O sistema de tickets ainda não está ativado no servidor. Para ativar, use o seguinte comando: \`/gerency ticket\`. Após ativar, use este comando novamente. :wink:`);
			...
			db.set(`rlt_${interaction.guildId}`, string);
		...
		try{
			...
			db.set(`tk${interaction.guildId}`, string);
		...
		try{
			...
			db.set(`sg${interaction.guildId}`, string);
