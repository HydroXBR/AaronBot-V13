...
exports.int = async(client, interaction) => {
	economia.findOne({_id: interaction.user.id}, (err, res) => {
	// Interaction content 
	...
	let dbPapel = res.papel_parede;
	let dbPapeis = res.papeis_conta;
	let dbMoney = res.money;
	let dbXp = res.xp;
	let dbLv = res.level;
	let dbWork = res.profissao;
	...
	if(command == "daily"){
		try{
			...
			const databaseDate = res.latest_daily;
		...
		try{
  		...
  		let member = interaction.options.getUser("user")
			let user = member.id;
  		let dinheiro = res.money;
  		let money = dinheiro == null ? 0 : dinheiro;
  		let moneyma = res.money;
		...
		economia.findOne({_id: user}, (err, userdb) => {

		if(!userdb) return messagerEphemeral(`❌ | Ops! Parece que o(a) usuário(a) nunca usou um comando meu, por isso, não tenho nenhum registro.`)
  		let d = Number(string);
  		...
  		let confirm = new MessageEmbed()
			...
			collector.on("collect", async i => {
				if (i.customId === "confirmTransaction") {
					res.money = (messagemoney - d)
					userdb.money = (money + d)
					res.save();
					userdb.save();
    ...
    return messagerEphemeral(`❌ | Ops! Desculpe, mas não foi possível completar a transação. Verifique se a marcação e a quantidade estão corretas, bem como se usuário está no servidor, mesmo. Lembrando, a sintaxe correta é: \`/eco pay @USER QUANTIDADE (EM NÚMERO INTEIRO)\``);
		...
		try{
			await interaction.deferReply();
			let work = res.profissao || `Nenhuma`;
  		let m = res.money;
  		let money = m == null ? 0 : m;
  		const databaseDate = res.latest_work;
			const datetime = new Date().toISOString().split("T")[0].split("-").reverse().join("/");
  		...
  		if(salario == 0){
    		interaction.editReply({ embeds: [embed.toJSON()], ephemeral: true });
			  ...
			  res.money = money + salario;
			  res.latest_work = datetime;
			  res.save();
  		};
		}catch(e){
			console.log(e);
		...
		try{
  		...
  		let money = m == null ? 0 : m;
			...
			for(var i = 0; i < prof.work.length; i++){
				...
				if(regex.test(name)){
					if (lv >= requiredLevel) {
		    		res.profissao = prof.work[i].name;
					res.save();
		...
		try{
			await interaction.deferReply();
			let especialidade = res.especialidade || `Nenhuma`;
			let esp = {
				"especialidades":[
					{
						"name": "Pregação",
		...
		try{
  		...
  		switch(string){
        	...
        	if(!papeis) {
			res.papeis_conta = papeisJSON.papeis[i].code;
			res.papel_parede = papeisJSON.papeis[i].code;
			res.money = (money - papeisJSON.papeis[i].price);
			res.save();
			...
			res.papeis_conta = papeis + papeisJSON.papeis[i].code;
      	  	res.papel_parede = papeisJSON.papeis[i].code;
      	    res.money = (money - papeisJSON.papeis[i].price);
			res.save();
          	return messager(`Parabéns! :smile: - Você comprou o papel de parede ${papeisJSON.papel[i].position} - ${papeisJSON.papel[i].name} com sucesso por \`${toBRL(papeisJSON.papel[i].price)}\` da conta Aaron.`);
		...
		try{
			...
			economia.findOne({_id: user.id}, (err, userdb) => {
				if(!userdb) return messagerEphemeral(`:confused: | Ops! ${user.id === interaction.user.id ? "Digite o comando novamente." : "O usuário não executou nenhum comando até o momento, por isso, não consigo encontrar."}`)
			let papel;
  		switch(userdb.papel_parede){
  			case null:
		...
		let img = jimp.read(papel)
  		...
  		let work = userdb.profissao || "Nenhuma";
  		let especialidade = userdb.especialidade || "Nenhuma";
  		let gema = userdb.gemas || 0;
		let carro = userdb.carros == null ? "" : userdb.carros.join('\n');
  		let objeto = userdb.objetos == null ? "" : userdb.objetos.join('\n');
		...
		try{
  		...
  		if (string == 'local'){
    		...
    		for(var e = 0; e < usersArray[0].length; e++){
					...
					console.log(user)
			const dat = economia.find({_id: usersArray[0][e]})
      		const u = {
        		user: {
